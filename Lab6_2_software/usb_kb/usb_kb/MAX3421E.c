...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	wr_data[1] = val;
	int return_code;
	return_code = alt_avalon_spi_command(SPI_MASTER_BASE, 0, 2, wr_data, 0, NULL, 0);
	if(return_code < 0)
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	BYTE wr_data[nbytes+1];
	wr_data[0] = reg + 2;
	for(int i=0; i<nbytes; i++){
		wr_data[i+1] = data[i];
	}
	int return_code;
	return_code = alt_avalon_spi_command(SPI_MASTER_BASE, 0, nbytes + 1, wr_data, 0, NULL, 0);
	if(return_code < 0)
		printf("ERROR SPI TX RET = %x \n" , return_code);
	return data + nbytes;
	//psuedocode:
...
BYTE MAXreg_rd(BYTE reg) {
	BYTE read_data;
	int return_code;
	return_code = alt_avalon_spi_command(SPI_MASTER_BASE, 0, 1, &reg, 2, &read_data, 0);
	if(return_code < 0)
		printf("ERROR SPI TX RET = %x \n" , return_code);
	return read_data;
	//psuedocode:
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	int return_code;
	return_code = alt_avalon_spi_command(SPI_MASTER_BASE, 0, 1, &reg, nbytes+1, data, 0);
	if(return_code < 0)
		printf("ERROR SPI TX RET = %x \n" , return_code);
	return data + nbytes;
	//psuedocode:
